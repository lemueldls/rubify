...
impl OutlinePen for PathPen {
    ...
    fn curve_to(&mut self, cx0: f32, cy0: f32, cx1: f32, cy1: f32, x: f32, y: f32) {
        self.path.curve_to((cx0 as f64, cy0 as f64), (cx1 as f64, cy1 as f64), (x as f64, y as f64));
    }
...
pub fn process_font_file(font_data: &[u8], pinyin_font_data: Option<&[u8]>) -> Result<Vec<u8>> {
    let file = FileRef::new(font_data).map_err(|e| anyhow!("Failed to parse input font file: {:?}", e))?;
    
    let fonts: Vec<_> = file.fonts().collect();
    if fonts.is_empty() {
        return Err(anyhow!("No fonts found in input file"));
    }
    let font_ref = fonts[0].clone().map_err(|e| anyhow!("Failed to load first font: {:?}", e))?;

    let pinyin_font_ref = if let Some(data) = pinyin_font_data {
        let pfile = FileRef::new(data).map_err(|e| anyhow!("Failed to parse pinyin font file: {:?}", e))?;
        let pfonts: Vec<_> = pfile.fonts().collect();
        if pfonts.is_empty() {
            return Err(anyhow!("No fonts found in pinyin font file"));
        }
        Some(pfonts[0].clone().map_err(|e| anyhow!("Failed to load first font from pinyin font file: {:?}", e))?)
    } else {
        None
    };
...
fn process_single_font(font: FontRef, pinyin_font: Option<FontRef>) -> Result<Vec<u8>> {
        ...
        if let Some(&ch) = gid_to_char.get(&gid) {
             if let Some(p) = ch.to_pinyin() {
                     ...
                     match p_charmap.map(pc) {
                         Some(pgid) if pgid.to_u32() != 0 => {
                             if let Some(pglyph) = p_outlines.get(pgid) {
                                 ...
                                 if pglyph.draw(skrifa::instance::Size::unscaled(), &mut ppen).is_ok() {
                 ...
                 if all_found && !pinyin_paths.is_empty() {
                     let orig_advance = hmtx.h_metrics().get(gid.to_u32() as usize).map(|m| m.advance.get()).unwrap_or(upem as u16) as f64;
                     
                     let mut total_pinyin_width = 0.0;
                     for (pgid, _) in &pinyin_paths {
                          let adv = p_hmtx.h_metrics().get(pgid.to_u32() as usize).map(|m| m.advance.get()).unwrap_or(p_upem as u16) as f64;
                     ...
                     for (pgid, mut p_path) in pinyin_paths {
                         let adv = p_hmtx.h_metrics().get(pgid.to_u32() as usize).map(|m| m.advance.get()).unwrap_or(p_upem as u16) as f64;
                         let xform = Affine::translate((current_x, target_y)) * Affine::scale(p_scale_factor);
    ...
    for record in font.table_directory.table_records() {
        let tag = record.tag();
        if tag == Tag::new(b"glyf") || tag == Tag::new(b"loca") { continue; }
...
pub fn subset_cjk(font_data: &[u8]) -> Result<Vec<u8>> {
    let file = frf::FileRef::new(font_data).map_err(|_| anyhow!("Failed to parse font for subsetting"))?;
    let font = file.fonts().next().context("No font found for subsetting")?.map_err(|e| anyhow!("Read error: {:?}", e))?;
    ...
    // ASCII (Basic Latin)
    for c in 0x0020..=0x007E {
        unicodes.insert(c);
    }
    
    // Pinyin chars (generic approach: just include all Latin Extended A/B which covers pinyin)
    for c in 0x0080..=0x024F {
        unicodes.insert(c);
    }
    // Also Combining Diacritical Marks: 0300–036F
    for c in 0x0300..=0x036F {
        unicodes.insert(c);
    }

    use fontcull_font_types::{GlyphId as FrfGlyphId, Tag as FrfTag, NameId as FrfNameId};
